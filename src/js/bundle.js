  ...
  const convertImages = function(event) {
    if (event.target.files.length > 0) {
      ...
      for (let i = 0; i < event.target.files.length; i++) {
        ...
        UimagesContainer.appendChild(originalDataSize);
        ...
        WimagesContainer.appendChild(convertedDataSize);

        // Process image and update elements
        processImage(event, file, originalImage, originalDataSize, convertedImage, convertedDataSize, sliderValue);
      }
    }
  };

  function processImage(event, file, originalImage, originalDataSize, convertedImage, convertedDataSize, sliderValue) {
    ...
    userImage.onload = function() {
      ...
      canvas.toBlob(function(blob) {
        ...
        reader.onloadend = function() {
          ...
          convertedDataSize.innerText = 'Converted File Size: ' + (webpImage.length / 1024).toFixed(2) + ' KB';
